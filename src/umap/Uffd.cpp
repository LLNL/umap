...
const bool Uffd::next_page_event( void )
{
...
void Uffd::enable_write_protect(
    char*
#ifndef UMAP_RO_MODE
    page_address
#endif
...
void Uffd::disable_write_protect(
    char*
#ifndef UMAP_RO_MODE
    page_address
#endif
...
void Uffd::copy_in_page_and_write_protect(char* data, char* page_address)
{
  UMAP_LOG(Debug, "(data = " << (void*) data << ")");
  struct uffdio_copy copy = {
      .dst = (uint64_t)page_address
...
void Uffd::register_with_uffd( void )
{
...
void Uffd::unregister_from_uffd(void)
{
  struct uffdio_register uffdio_register = {
      .range = { .start = (uint64_t)m_region, .len = m_region_size }
    , .mode = 0
